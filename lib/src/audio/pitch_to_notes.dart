  ...
  static List<DiscreteNote> consolidate(List<PitchPoint> points) {
    ...
    for (final p in points) {
      ...
      if (curNote == null) {
        ...
        curStart = t;
        curEnd = t;
      } else if (n == curNote && (t - curEnd) <= 200) {  // Increased from 120ms to 200ms for more lenient merging
        // same note within short gap -> extend
        curEnd = t;
      } else {
        final dur = math.max(0, curEnd - curStart);
        if (dur >= 80) {  // Reduced from 100ms to 80ms minimum duration
          merged.add(
    ...
    if (curNote != null) {
      if (dur >= 80) {
        merged.add(DiscreteNote(note: curNote, startMs: curStart, durationMs: dur));
      } else {
        shortNotesFiltered++;
      }
    }
    ...
    print('  Retention rate: ${((merged.length / rawNotes) * 100).toStringAsFixed(1)}%');
    
    if (merged.isNotEmpty) {
      print('  Final notes: ${merged.map((n) => '${n.note}(${n.durationMs}ms)').join(', ')}');
      final noteCount = <String, int>{};
      for (final n in merged) {
        noteCount[n.note] = (noteCount[n.note] ?? 0) + 1;
      }
      print('  Note summary: ${noteCount.entries.map((e) => '${e.key}:${e.value}').join(', ')}');
    }
