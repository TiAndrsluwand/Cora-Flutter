...
class DetectedNote {
  ...
  final int startTime;
  final int duration;
  DetectedNote({required this.note, required this.startTime, required this.duration});
}
...
class ChordSuggestionEngine {
  ...
  List<ChordProgressionSuggestion> suggest(List<DetectedNote> melody, String keyLabel, bool isMinor) {
    if (melody.isEmpty || keyLabel == 'Unknown') return [];
    final keyRoot = isMinor ? keyLabel.replaceAll('m', '') : keyLabel;
    final scaleType = isMinor ? MT.SCALE_TYPES.NATURAL_MINOR : MT.SCALE_TYPES.MAJOR;
  ...
  bool _isPhraseBoundary(List<DetectedNote> melody, int pos) {
    ...
    final pitchChange = (MT.getNoteIndex(cur.note) - MT.getNoteIndex(prev.note)).abs();
  ...
  List<ChordProgressionSuggestion> _generateProgressions(
    ...
    final patterns = scaleType == MT.SCALE_TYPES.MAJOR
      ? [
          {'name':'Basic I-IV-V','nums':['I','IV','V']},
          {'name':'Pop I-V-vi-IV','nums':['I','V','vi','IV']},
          {'name':'Classic I-vi-IV-V','nums':['I','vi','IV','V']},
          {'name':'ii-V-I','nums':['ii','V','I']},
          {'name':'Turnaround I-vi-ii-V','nums':['I','vi','ii','V']},
          {'name':'vi-ii-V-I','nums':['vi','ii','V','I']},
        ]
      : [
          {'name':'Natural Minor i-iv-v','nums':['i','iv','v']},
          {'name':'Harmonic Minor i-iv-V','nums':['i','iv','V']},
          {'name':'Epic i-VI-III-VII','nums':['i','VI','III','VII']},
          {'name':'Minor iiø-V-i (approx)','nums':['ii','V','i']},
        ];

    final all = [...patterns];
    for (var i=0;i<options.preferredProgressions.length;i++) {
      all.add({'name':'Custom ${i+1}','nums':options.preferredProgressions[i]});
    }
  ...
  List<ChordSuggestion> _adaptToSegments(
    ...
    while (repeated.length < segments.length) { repeated.addAll(chordProg); }
  ...
  double _chordWeight(MT.DiatonicChord chord, List<DetectedNote> segment, List<String> scale) {
    ...
    for (final n in segment) {
      ...
      final onStrongBeat = (posMs % 500) < 120; // rough 120ms window around 0.5s grid

      double noteScore;
      if (MT.isNoteInChord(norm, chordNotes)) noteScore = 2.2;
      else if (MT.isNoteInScale(norm, scale)) noteScore = 1.0;
      else noteScore = 0.1; // chromatic
      ...
      if (n.duration > 400) noteScore += 0.4; // sustained tones are harmonically salient
  ...
  double _scoreProgression(List<ChordSuggestion> chords, List<List<DetectedNote>> _segments, List<String> scale) {
    ...
    for (var i=1;i<chords.length;i++) {
      ...
      if (i == chords.length - 1) {
        if (degB == 1) score += 0.6; // authentic/plagal resolution
