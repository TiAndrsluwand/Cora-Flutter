...
class _RecorderPageState extends State<RecorderPage> with TickerProviderStateMixin {
  ...
  Color _getRecordButtonTextColor() {
    return Colors.white;
  }

  String _getRecordButtonText() {
    if (_waitingForCountIn) return 'CANCEL';
    if (_isRecording) return 'STOP';
    return 'RECORD';
  }

  String _getStatusText() {
    if (_waitingForCountIn) {
      return 'Get ready... Count-in starting!';
    }
    if (_isRecording) {
      return 'Recording... ${_seconds}s / ${_maxSeconds}s max';
    }
    return 'Ready to record up to ${_maxSeconds} seconds';
  }

  List<Widget> _buildMetronomeControls() {
    return [
      Padding(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const Spacer(),
                IconButton(
                  onPressed: () => _updateBpm(MetronomePlayer.bpm - 5),
                ...
                SizedBox(
                  width: 60,
                  child: TextField(
                    controller: TextEditingController(text: MetronomePlayer.bpm.toString()),
            ...
            Slider(
              value: MetronomePlayer.bpm.toDouble(),
              min: 60,
              max: 200,
              divisions: 140,
              onChanged: (value) => _updateBpm(value.round()),
              label: MetronomePlayer.bpm.toString(),
            ),
            const SizedBox(height: 16),
            // Time Signature
            Row(
              children: [
                const Text('Time Signature:', style: TextStyle(fontWeight: FontWeight.w500)),
                const Spacer(),
                DropdownButton<TimeSignature>(
                  value: MetronomePlayer.timeSignature,
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Metronome will count-in ${MetronomePlayer.timeSignature.numerator} beats before recording.',
                      style: TextStyle(
                        fontSize: 13,
  ...
  Widget _buildCurrentStateContent() {
      ...
      return Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.timer, size: 48, color: Colors.amber.shade700),
    ...
    // Results state
    if (_path != null && (_detectedKey != null || _suggestions.isNotEmpty)) {
      return _buildResultsContent();
    }

    // Idle state with playback controls
    if (_path != null) {
        ...
        child: Column(
          ...
          children: [
            ...
            if (_playbackDuration != null && _playbackDuration!.inMilliseconds > 0) ...[
              ...
              const SizedBox(height: 8),
              Text(
                '${_formatTime(_playbackPosition)} / ${_formatTime(_playbackDuration!)}${_isPlayingBack ? '  (playing)' : ''}',
                style: const TextStyle(fontSize: 12, color: Colors.grey),
              ),
            ],
          ],
          ...
          children: [
            Row(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.music_note, color: Colors.green.shade600),
                    const SizedBox(width: 8),
                    Text(
                      'Analysis Results',
                      style: TextStyle(
            ...
            if (_suggestions.isNotEmpty) ...[
              ...
              // Piano Keyboard with Visibility
              Visibility(
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'No chord progressions found. Try recording a clearer melody.',
                        style: TextStyle(
